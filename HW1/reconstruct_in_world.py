"""
Usage: 
python reconstruct_in_world.py --test_scene=Data_collection/first_floor --floor=1 --use_open3d=1

python reconstruct_in_world.py --test_scene=Data_collection/second_floor --floor=2 --use_open3d=1
"""
import numpy as np
import cv2
import argparse
...
if __name__ == '__main__':
    ...
    rotation_mtx[:3, :3] = euler_from_quaternion(df_gt.iloc[0,3], df_gt.iloc[0,4], df_gt.iloc[0,5], df_gt.iloc[0,6]) * scale_factor
    points = [[df_gt.iloc[i,0], df_gt.iloc[i,1], df_gt.iloc[i,2]] for i in range(len(df_gt))]
    lines = [[i,i+1] for i in range(num_pcd-1)]
    colors = [[0, 0, 0] for i in range(len(lines))]
    print(len(points))
    translation = np.array([points[0][0], points[0][1], points[0][2]])
    ...
    for i in range(len(trans_mtx_list)):
        point_temp = rotation_mtx @ trans_mtx_list[i] @ np.array([0.0, 0.0, 0.0, 1.0])
        points.append([point_temp[0], point_temp[1], point_temp[2]])
    point_temp = rotation_mtx @ np.array([0.0, 0.0, 0.0, 1.0])
    points.append([point_temp[0], point_temp[1], point_temp[2]])
    print(len(points))
    points.reverse()
    # points = list(((np.array(points).T)).T * scale_factor + translation)
    ...
    for i in range(num_pcd):
        pcd_list[i].transform(rotation_mtx)
    for i in range(num_pcd):
